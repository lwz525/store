...
class Account
{
    ...
    public function register(&$error_msg)
    {
        ...
        {
            // 查重
            $sql = 'SELECT COUNT(*) as num from users WHERE uid=:uid';
            $stmt = $dbh->prepare($sql);
            $stmt->bindParam(':uid', $this->uid, PDO::PARAM_STR);
            $result = $stmt->execute();
			//$result = $stmt->fetch();
            ...
            // 添加用户
            $sql = 'INSERT INTO users set uid=:uid, pwd=:pwd, register_time=:registerTime';
            $stmt = $dbh->prepare($sql);
            $stmt->bindParam(':uid', $this->uid, PDO::PARAM_STR);
            // 加密密码
...
//            $res = $dbh->lastInsertId();
    ...
    public function login(&$error_msg)
    {
        ...
        try
        {
            $sql = 'UPDATE users SET `state` = 1, token=:token, user_sig=:userSig,  login_time=:loginTime, last_request_time=:lastRequestTime WHERE uid=:uid';
            $stmt = $dbh->prepare($sql);
    ...
    public function logout(&$error_msg)
    {
        ...
        try
        {    
            $sql = 'UPDATE users SET `state` = 0, token=null, logout_time=:logoutTime WHERE token=:token';
            $stmt = $dbh->prepare($sql);         
    ...
    public function kickout(&$error_msg, $sdkappid, $admin_uid, $admin_sig, $hold_sig)
    {
        ...
        if($hold_sig === false){
            ...
            try
            {
                $sql = 'UPDATE users SET user_sig=null WHERE uid=:uid';
                $stmt = $dbh->prepare($sql);
                $stmt->bindParam(':uid', $this->uid, PDO::PARAM_STR);
                $result = $stmt->execute();
                if (!$result)
    ...
    public function updateLastRequestTime(&$error_msg)
    {
        ...
        try
        {    
            $sql = 'UPDATE users SET last_request_time=:lastRequestTime WHERE token=:token';
            $stmt = $dbh->prepare($sql);         
